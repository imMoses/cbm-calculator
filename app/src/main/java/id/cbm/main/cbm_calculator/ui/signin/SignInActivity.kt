            ...
            viewModel.signInResponse.observe(this) { result ->
                val apiResultHandler = ApiResultHandler<BaseApiResponse<SignInResponse>>(
                    ...
                    onSuccess = { response ->
                        ...
                        response?.let { safeResp ->
                            if (safeResp.success == true) {
                                if (safeResp.code == 200) {
                                    safeResp.data?.let { safeData ->

                                        // save token logged in
                                        LocalDataController.General().putModel(
                                            context = this,
                                ...
                                Toast.makeText(this, "Response is: ${safeResp.success} - ${safeResp.code}", Toast.LENGTH_SHORT).show()
                            }
                        }
                    },
                    onFailure = {
